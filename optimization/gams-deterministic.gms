...
* mean total network demand in KL
Parameter D(p) /
die 181000
pet 64000 /;
...
obj1..              sum((p,r,i,j)$(not sameas (i,j)), TC(r,i,j)*X(p,r,i,j)/1000) =e= z('cost');
obj2..              sum((p,r,i,j)$(not sameas (i,j)), LC(p,r,i,j)*X(p,r,i,j)/100) =e= z('loss');

c1(p,m)..         sum((r,j)$(not sameas (m,j)), (X(p,r,m,j) - X(p,r,j,m))) =l= A(p,m);
c2(p,n)..         sum((r,i)$(not sameas (i,n)), (X(p,r,i,n) - X(p,r,n,i))) =g= alpha(p,n)*D(p);
c3(p,n)..         sum((r,i)$(not sameas (i,n)), (X(p,r,i,n) - X(p,r,n,i))) =l= alpha(p,n)*D(p) + U(p,n) + V(p,n);
c4(p,o,i,j)..     X(p,o,i,j)$(not sameas (i,j)) =l= H(p,i,j)*F(i,j);
c5(p)..           sum(n, U(p,n) + V(p,n)) =g= D(p);
c6(p,n)..         U(p,n) + V(p,n) =g= beta(p,n)*alpha(p,n)*D(p);
c7(p,n)..         U(p,n) + V(p,n) =g= gamma(p)*(sum((r,i), X(p,r,i,n)));
c8(n)..           sum(p, V(p,n)) =l= W(n);
c9(p,r,i,j)..     X(p,r,i,j)$(not sameas (i,j)) =g= 0;
c10(p,n)..        V(p,n) =g= 0;
...
*     The jump is calculated for the innermost objective function (km=1)
      ...
      put /;
      put ' ':5 'X' /
      loop((p,r,i,j)$(abs(X.l(p,r,i,j))>1e-6), put ' ':7 p.tl r.tl i.tl j.tl X.l(p,r,i,j) /);
      put ' ':5 'V' /
      loop((p,n)$(abs(V.l(p,n))>1e-6), put ' ':7 p.tl n.tl V.l(p,n) /);
   );
